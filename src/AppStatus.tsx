...
function WorkspaceBar(){
  ...
  return <div className="workspace-bar" aria-label="Cambiar espacio de trabajo">
    <b>GATRIVI CRM</b>
    <div>{tenants.map(tenant=><button key={tenant} type="button" className={tenant===session.tenant?'selected':''} onClick={()=>tenant!==session.tenant&&goToTenant(tenant)}>{labels[tenant]??tenant}</button>)}</div>
  </div>;
}
  ...
  return <div className="release-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)dismiss()}}>
    <section className="release-card" role="dialog" aria-modal="true" aria-labelledby="release-title">
      ...
      <div className="release-actions">
        {session.tenant==='jobs'&&<button className="secondary" type="button" onClick={openSales}>Abrir Ventas</button>}
