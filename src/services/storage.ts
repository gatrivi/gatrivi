...
const normalize=(data:Partial<CrmData>):CrmData=>({
...
export function seed(tenant='demo'):CrmData {
  ...
  if(tenant==='jobs'||tenant==='personal')return {contacts:[],stages,deals:[],tasks:[],messages:[],proposals:[]};
  ...
  const tasks=['Llamar a Ana','Enviar propuesta a Bruno','Revisar contrato','Preparar reunión'].map((title,i)=>({id:`task-${i+1}`,tenantId:tenant,contactId:contacts[i].id,dealId:deals[i]?.id,title,dueDate:new Date(Date.now()+(i+1)*86400000).toISOString().slice(0,10),done:i===3,createdAt:now}));
  return {contacts,stages,deals,tasks,messages:[],proposals:[]};
}

export function load(tenant:string):CrmData {
  try{
    const raw=localStorage.getItem(key(tenant));
    if(raw)return normalize(JSON.parse(raw) as Partial<CrmData>);
...
export function importJobApplication(input:JobImport):CrmData {
  ...
  if(stageName==='Aplicado'&&!tasks.some(task=>task.id===followupId)){
    tasks=[...tasks,{id:followupId,tenantId:tenant,contactId,dealId,title:`Follow-up: ${company} — ${title}`,dueDate:new Date(Date.now()+7*86400000).toISOString().slice(0,10),done:false,createdAt:now}];
  }
  if(stageName==='Rechazado')tasks=tasks.map(task=>task.id===followupId?{...task,done:true}:task);

  const next={...data,contacts,stages,deals,tasks};
  save(tenant,next);
  return next;
}
